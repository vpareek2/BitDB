...
char* get_db_query(const char* user_input) {
    ...
    sprintf(command, "python3 llama/lora.py \"%s\"", user_input);
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        // Check for special keyword "Ada"
        if (strncmp(input_buffer->buffer, "Ada ", 4) == 0) {  // Changed from "llama " to "Ada "
            // Extract the part after "Ada"
            char* natural_language_input = input_buffer->buffer + 4;  // Changed offset from 6 to 4

            // Convert natural language input to database query
            char* db_query = get_db_query(natural_language_input);
            printf("Received DB Query: %s\n", db_query); // Debug print

            strncpy(input_buffer->buffer, db_query, input_buffer->buffer_length - 1);
